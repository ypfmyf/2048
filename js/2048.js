...
function setWidthHeight() {
  ...
  var height = window.innerHeight - $('#pid').height();

  //约定base_marginleft为cell_width的1/4
  // (base_marginleft*5)+(16*base_marginleft) = width
  //约定base_margintop为cell_height的1/4
  base_margin_left = (1 / 21) * width;
  cell_width = 4 * base_margin_left;

  base_margin_top = (1 / 21) * height;
  cell_height = 4 * base_margin_top;
  ...
  for (var r = 0; r < 4; r++) {
    //遍历一行中的列
    for (var c = 0; c < 4; c++) {
      //计算出来当前位置对应的top和left值
      var topValue =
          base_margin_top * (r + 1) + cell_height * r;
      var leftValue =
          base_margin_left * (c + 1) + cell_width * c;
      //查找行为r列为c的元素
      $('[id $="' + r + c + '"]').css({
        top: topValue,
        left: leftValue
    ...
    for (var r = 0; r < this.RN; r++) {
      for (var c = 0; c < this.CN; c++) {
        ...
        if (this.data[r][c] == 0) {
          ...
          //拿到base_margin_left以及base_margin_top值
          var topValue =
              base_margin_top*(r+1)+cell_height*r;
          var leftValue =
              base_margin_left*(c+1)+cell_width*c;
